    ...
    "스커크": characterDataSchema(
        ...
        activeSkill={
            ...
            "극악기·섬": activeSkillSchema(
                baseFightProp=skillBaseFightPropSchema(),
                additionalAttack=[
                    additionalAttackSchema(name="일곱빛 섬광(일반공격)", type="nomal", baseFightProp=damageBaseFightPropSchema(ATTACK=1, element=["ice"])),
                    additionalAttackSchema(name="일곱빛 섬광(강공격)", type="charge", baseFightProp=damageBaseFightPropSchema(ATTACK=1, element=["ice"])),
                    additionalAttackSchema(name="일곱빛 섬광(낙하공격)", type="falling", baseFightProp=damageBaseFightPropSchema(ATTACK=1, element=["ice"])),
                ],
            ),
        ...
        constellation=[
            ...
            contellationSchema(
                ...
                options=[skillConstellationOptionSchema(type=skillConstellationType.stack, maxStack=3, label="극악기·진 발동")],
            ),
            contellationSchema(name="악연", description="원소 폭발 레벨 +3"),
            contellationSchema(name="멸류", description="죽음의 강 효과 스텍마다 공격력 증가"),
            contellationSchema(name="소망", description="원소 전투 스킬 레벨 +3"),
            contellationSchema(
                name="근원",

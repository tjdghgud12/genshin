...
info = {
    "푸리나": characterDataSchema(
        passiveSkill={
            "끝없는 왈츠": passiveSkillSchema(
                description="필드 위 캐릭터가 치유 받을 시 푸리나의 치유가 아닌 동시에 회복량이 초과된 경우 주변 파티 내 캐릭터 hp 회복", unlockLevel=1
            ),
            "고독한 독백": passiveSkillSchema(description="푸리나의 최대 hp 1000pt 당 원소 전투 스킬 피해 증가", unlockLevel=4),
        },
        activeSkill={
        ...
        constellation=[
            contellationSchema(name="「사랑은 애걸해도 길들일 수 없는 새」", description="원소 폭발 발동 시 무대 열기 +150pt 및 무대 열기 최대치 +100pt"),
            contellationSchema(name="「여자의 마음은 흔들리는 부평초」", description="원소 폭발 발동 시 지속 시간 동인 무대 열기 당 hp최대치 증가"),
            contellationSchema(name="「내 이름은 그 누구도 모르리라」", description="원소 폭발 레벨 +3"),
            contellationSchema(name="「저승에서 느낀 삶의 소중함!」", description="원소 전투 스킬이 적 명중 또는 파티원 회복 시 원소 에너지 획득"),
            contellationSchema(name="「난 알았노라, 그대의 이름은…!」", description="원소 전투 스킬 레벨 +3"),
            contellationSchema(
                name="「모두 사랑의 축배를 들렴!」",
    ...
    "느비예트": characterDataSchema(
        ...
        constellation=[
            contellationSchema(name="위대한 제정", description="생존한 용의 영광을 1 스택 획득 및 강공격 시 경직 저항력 증가"),
            contellationSchema(name="법의 계율", description="생존한 용의 영광 1스텍 당 강공격 치명타 피해 14% 증가"),
            contellationSchema(name="고대의 의제", description="일반공격 스킬 레벨 +3"),
            contellationSchema(name="연민의 왕관", description="치유 받을 시 원천의 방울 1개 생성"),
            contellationSchema(name="정의의 판결", description="원소 폭발 레벨 +3"),
            contellationSchema(
                name="분노의 보상",
    ...
    "에스코피에": characterDataSchema(
        passiveSkill={
            "밥이 보약": passiveSkillSchema(description="원소 폭발 발동 후 파티 내 모든 캐릭터 hp 회복", unlockLevel=1),
        ...
        constellation=[
            contellationSchema(
                ...
                options=[skillConstellationOptionSchema(type=skillConstellationType.toggle, label="원소 전투 스킬 또는 원소 폭발 발동")],
            ),
            contellationSchema(
                name="예술의 경지에 이른 스튜",
                description="원소 전투 스킬 발동 시 피해를 에스코피에의 공격력의 240%만큼 증가시키는 즉석 요리 스텍 획득",
                options=[skillConstellationOptionSchema(type=skillConstellationType.toggle, label="즉석 요리 스텍")],
            ),
            contellationSchema(name="캐러멜화의 마법", description="원소 전투 스킬 레벨 +3"),
            contellationSchema(name="로즈마리 비밀 레시피", description="식이 요법 지속시간 증가 및 식이 요법으로 치유 시 원소 에너지 획득"),
            contellationSchema(name="다채로운 소스의 교향곡", description="원소 폭발 레벨 +3"),
            contellationSchema(
                name="무지갯빛 티타임",
    ...
    "클로린드": characterDataSchema(
        ...
        constellation=[
            ...
            contellationSchema(name="「지금부터 긴 밤의 위험에 맞선다」", description="밤을 가르는 불꽃의 효과 강화"),
            contellationSchema(name="「난 낮의 맹세를 명심하고」", description="원소 전투 스킬 레벨 +3"),
            contellationSchema(
                name="「눈물, 생명, 사랑을 간직하며」",
            ...
            contellationSchema(name="「언젠가 찾아올 여명을 믿겠다」", description="원소 폭발 레벨 +3"),
            contellationSchema(
    ...
    "나비아": characterDataSchema(
        ...
        constellation=[
            contellationSchema(name="숙녀의 거리감 수칙", description="결정 파편 소모할 때 소모 갯수 당 원소 에너지 회복 및 원소 폭발 쿨타임 감소"),
            contellationSchema(
                name="통솔자의 승승장구",
            ...
            contellationSchema(name="경영자의 넓은 시야", description="원소 전투 스킬 레벨 +3"),
            contellationSchema(
                name="맹세자의 엄격함",
            ...
            contellationSchema(name="협상가의 단호함", description="원소 폭발 레벨 +3"),
            contellationSchema(
                name="보스의 기민한 수완",

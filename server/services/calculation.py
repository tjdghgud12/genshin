...
async def damageCalculation(characterInfo: requestCharacterInfoSchema, additionalFightProp: fightPropSchema) -> responseCalculationResult:
    ...
    for skill in characterInfo.activeSkill:
        attacks = [*attacks, *((k, v, False) for k, v in skill.baseFightProp.model_dump().items() if v is not None)]
    ...
    for attackType, baseFightProp, customName in attacks:
        ...
        baseFightPropList = [getattr(fightProp, baseFightPropKeyMap[base.name]) * baseFightProp[base.name] for base in baseFightPropKeyMap if baseFightProp[base.name] is not None]
        ...
        if finalAttackPoint:
            ...
            if useEelementalAttack:
                ...
                elementAddPoint = (
                    ...
                    + getattr(fightProp, f"FIGHT_PROP_{attackElement.upper()}_ADD_POINT", 0.0)
                    + getattr(fightProp, f"FIGHT_PROP_{key}_{attackElement.upper()}_ADD_POINT", 0.0)
                )
                elementAddHurt = getattr(fightProp, f"FIGHT_PROP_{key}_{attackElement.upper()}_ADD_HURT", 0.0) + getattr(fightProp, f"FIGHT_PROP_{attackElement.upper()}_ADD_HURT")
